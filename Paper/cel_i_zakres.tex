...
\begin{itemize}
    \item Umożliwienie stabilnego \textbf{wyświetlania obrazu w czasie rzeczywistym} w przeglądarce internetowej. Weryfikacja nastąpi poprzez pomiar \textbf{opóźnienia strumienia wideo ($Latency$)} oraz wskaźnika \textbf{klatek na sekundę (FPS)}, celem osiągnięcia płynności monitoringu porównywalnej z dedykowanymi systemami VMS.
    \item Zdalne sterowanie kluczowymi funkcjami kamery, w tym \textbf{ruchem PTZ} (Pan/Tilt/Zoom), za pośrednictwem własnego API.
    ...
    \item Osiągnięcie architektury gotowej do \textbf{integracji} z otwartymi platformami automatyki domowej (np. \texttt{Home Assistant}).
\end{itemize}
